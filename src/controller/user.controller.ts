...
import {
  createChain,
  handle400Request,
  handle404Request, handleNotFoundEndpoint
} from "../utils/http-request-handler";
...
export const userController =
  ...
  async (
    ...
    if (method === HttpMethod.GET && !userId) {
      const result = await service.get().catch((error) => {
    ...
    if (method === HttpMethod.POST && body && userId) {
      handleNotFoundEndpoint(res);
    }
